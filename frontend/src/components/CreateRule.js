...
const CreateRule = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (/[^a-zA-Z0-9\s&|=><()'\"!]/g.test(ruleString)) {
      setMessage(
        "Error: Invalid symbols detected. Only alphanumeric characters, spaces, and symbols like &, |, =, >, <, (), '\" and ! are allowed."
      );
    ...
    if (!/[><=!]/.test(ruleString)) {
      setMessage(
        "Error: The rule must contain at least one comparison operator (e.g., >, <, =, >=, !=, etc.)."
      );
      setMessageType("error");
      return;
    }
    ...
    if (/\band\b|\bor\b/.test(ruleString)) {
      setMessage(
        "Error: Please use 'AND' or '&&' and 'OR' or '||' instead of 'and' or 'or'."
      );
      setMessageType("error");
      return;
    }
    ...
    if (/[^&]&(?!&)|[^|]\|(?!\|)/.test(formattedRuleString)) {
      setMessage(
        "Error: Please use 'AND' and 'OR' instead of single '&' and '|'."
      );
      setMessageType("error");
      return;
    }
    ...
    formattedRuleString = formattedRuleString
      .replace(/&&/g, "AND")
      .replace(/\|\|/g, "OR");
    ...
    try {
      const response = await createRule({
        name,
        rule_string: formattedRuleString,
      });
